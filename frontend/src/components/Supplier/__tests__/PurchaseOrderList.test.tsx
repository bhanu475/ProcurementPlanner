...
vi.mock('../PurchaseOrderModal', () => ({
  default: ({ purchaseOrder, onClose, onUpdate }: any) => (
    <div data-testid="purchase-order-modal">
      <p>Modal for {purchaseOrder.purchaseOrderNumber}</p>
      <button onClick={onClose}>Close</button>
      <button onClick={() => onUpdate({ ...purchaseOrder, status: 'Confirmed' })}>
        Update
      </button>
    </div>
  ),
...
describe('PurchaseOrderList', () => {
  ...
  it('updates order when modal triggers update', async () => {
    ...
    // Modal should close
    expect(screen.queryByTestId('purchase-order-modal')).not.toBeInTheDocument();
  });
  ...
  it('displays correct status colors', async () => {
    ...
    await waitFor(() => {
      ...
      const sentToSupplierStatus = statusElements.find(el => el.closest('span')?.classList.contains('text-orange-800'));
      
      const confirmedElements = screen.getAllByText('Confirmed');
      const confirmedStatus = confirmedElements.find(el => el.closest('span')?.classList.contains('text-green-800'));
      
      const inProductionStatus = screen.getByText('InProduction');

      expect(sentToSupplierStatus?.closest('span')).toHaveClass('text-orange-800');
